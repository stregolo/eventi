...
def generate_m3u8(matches):
    ...
    with open(fileVlc, 'w', encoding='utf-8') as fileVLC, open(fileTiv, 'w', encoding='utf-8') as fileTIV:
        ...
        for channel in matches.values():
            ...
            fileVLC.write(f'#EXTVLCOPT:http-referrer=https://daddylive.dad/\n')
            ...
            fileVLC.write(f'#EXTVLCOPT:http-origin=https://daddylive.dad\n')
            fileVLC.write(f"{url}\n")
            
            fileTIV.write(f"#EXTINF:-1 tvg-id=\"{tvg_id}\" tvg-name=\"{channel_name}\" tvg-logo=\"{tvicon_path}\" group-title=\"{category}\", {channel_name}\n")
            fileTIV.write(f"{url}|Referer=\"https://daddylive.dad/\"|User-Agent=\"Mozilla/5.0 iPhone; CPU iPhone OS 17_6_0 like Mac OS X AppleWebKit/605.2.10 KHTML, like Gecko Version/17.6.0 Mobile/16F152 Safari/605.2\"|Origin=\"https://daddylive.dad\"\n")
